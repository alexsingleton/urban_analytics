...
plot(insurance.data$pct_insur, insurance.data$pct_unemp, 
     xlab = "% Insured",  
     ylab = "% Unemployed",
     main = "Linear Regression of Population Insured in Phoenix, Arizona",
...
A queen matrix means that associations are calculated diagonally as well (8 directions); in the case of polygons, a common point will suffice. We can visualise the associations by plotting a line between each neighboroing polygon centroid.
...
Using the spatial matrix we can calculate the _I_ statistic; based on the p-value below we can reject the null hypothesis that there is no spatial autocorrelation in our residuals, which suggests we will have to use a different approach.
...
The spatial regression model approach (often called _spatial lag_ or _spatial error_ models) is very similar to a traditional time-series regression, however correaltion coefficients are adjusted to include spatial associations. The first one account for spatial dependencies as part of the dependent variable $y$ while the second as part of the error $varepsilon$, depending whether spatial autocorrelation can be regarded as substantive and nuisance respectively.
